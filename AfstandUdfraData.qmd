...
Et første bud kunne være at bestemme den [euklidske afstand](AfstandeMellemPunkteriPlanen.qmd) mellem de to punkter -- det der bare svarer til at bruge Pythagoras. Gør vi det får vi følgende afstandsmål mellem punkterne $(v_1,h_1)$ og $(v_2,h_2)$

$$\sqrt{(v_2-v_1)^2+(h_2-h_1)^2}$$

Vi prøver at regne lidt på det, og forestille os at tre personer er givet som datapunkter i nedenstående tabel.
...
Lad os sige, at vi betrager datapunkter $(x_i,y_i)$ i planen, hvor $x$-værdierne ligger mellem $a$ og
...
Det får den betydning, at hvis $x$-værdierne f.eks. kan varierer i et langt bredere interval end $y$-værdierne (dvs. at $b-a>d-c$), så bliver forskellen på $x$-værdierne i ovenstående afstandsmål skaleret mere ned (fordi vi kommer til at dividere med et større tal). En anden måde at forstå dette afstandsmål på, er ved at erstatte hvert punkt $(x_i,y_i)$ med et nyt skaleret punkt:
...
![Datapunkterne fra @fig-min_max_skalering, men hvor alle punkter er blevet skaleret ved at bruge formlen i (@eq-norm).](images/fig_min_max_skalering2.png){#fig-min_max_skalering2}
...
Bemærk, at det præcis er afstandsmålet i (@eq-min_max_skalering).  

## Mindre naivt, mere bøvlet: Feature scaling

Den skalering vi har præsenteret i (@eq-norm) benytter ikke som sådan information om data, men kun om den mindste og største værdi, som henholdsvis $x$- og $y$-værdier ligger imellem (nemlig $a$ og $b$ for $x$-værdierne og $c$ og $d$ for $y$-værdierne).

Et alternativ til dette er at bruge data til at bestemme
skaleringen. Dette kaldes for *feature scaling*, når vi arbejder med
perceptroner eller neurale netværk. Hvis de data, der skal læres fra -- det vil sige
træningsdata -- er 
$$(x_1,y_1), (x_2,y_2),\ldots, (x_n,y_n)$$ 
så skalerer vi langs førsteaksen ved, at

* udregne et estimat for middelværdien af $x$:
    $$\bar{x}=\frac{x_1 + x_2 + \cdots + x_n}{n}=\frac{\Sigma_{i=1}^nx_i}{n}$$
...
$$ Z = \frac{X-\mu}{\sigma}$$
Derfor vil det også være sådan, at hvis de oprindelige data er normalfordelte, så vil de nye feature skalerede data være standard normalfordelte (dvs. normalfordelte med middelværdi $0$ og spredning $1$). Heraf følger også, at cirka $95 \%$ af de feature skalerede data vil ligge mellem $-2$ og $2$.
...
Datapunkterne fra tabellen ses indtegnet i @fig-bolig_milk_raw. De tre første familier $A$, $B$ og $C$ fra det tidligere eksempel er markeret. Bemærk, at vi tidligere har beregnet, at afstanden mellem $A$ og $B$ er den samme som afstanden mellem $A$ og $C$. Det ser måske ikke umiddelbart sådan ud i @fig-bolig_milk_raw, men det skyldes, at enheden på de to akser er forskellige.

![Datapunkterne fra eksemplet omkring udgifter til bolig og mælk.](images/fig_bolig_milk_raw.png){#fig-bolig_milk_raw}
