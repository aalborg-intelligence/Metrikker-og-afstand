...
Tilsvarende estimeres middelværdi og spredning for $y$ og feature scaling udregne 
$$\hat{y}_i= \frac{y_i-\bar{y}}{s_y}$$

Euklidisk afstand mellem disse nye punkter $(\hat{x}_i,\hat{y}_i)$ og  $(\hat{x}_j,\hat{y}_j)$ er 

$$\sqrt{(\frac{x_i-\bar{x}}{s_x}-\frac{x_j-\bar{x}}{s_x})^2 +(\frac{y_i-\bar{y}}{s_y}-\frac{y_j-\bar{y}}{s_y})^2}=\sqrt{(\frac{x_i-x_j}{s_x})^2+(\frac{y_i-y_j}{s_y})^2}$$
